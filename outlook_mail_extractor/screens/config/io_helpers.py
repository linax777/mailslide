...
def dump_yaml_text(payload: dict[str, Any]) -> str:
    """Serialize payload into YAML text.

    Args:
        payload: YAML object payload.

    Returns:
        YAML string with stable style for editor/config files.
    """
...
def write_yaml_with_backup(
    ...
    *,
    ...
    """Write YAML atomically and create backup when original exists.

    Args:
        target_path: File to write.
        payload: YAML object payload.
        backup_path: Optional backup file path. Defaults to `<name>.yaml.bak`.
