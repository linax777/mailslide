...
Language: English (`GUIDE.en.md`) | Traditional Chinese (`GUIDE.md`)
...
## Quick Start (Developers / source mode)
...
> This option is for source mode only (repo checkout with local `config/`).
> If you installed via `uv tool install mailslide`, use **About -> Initialize Config** instead.
...
- `uv tool` install mode: run `mailslide ...`
- Source mode: run `uv run mailslide ...`
...
> Path rule: relative `output_dir` / `output_file` values are resolved from the active config directory (the directory containing `config/config.yaml`).
