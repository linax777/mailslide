...
For the English version, see `GUIDE.en.md`.
...
> ⚠️ 此方法僅適用於原始碼模式（repo 內有 `config/` 目錄）。
> 若使用 `uv tool install mailslide`，請改用 About 的 **初始化設定**。
...
> 路徑規則：`output_dir` / `output_file` 若使用相對路徑，會以當前設定檔目錄（`config/config.yaml` 所在目錄）為基準。
...
- `outlook_open_command`：可複製到 PowerShell 執行的開信命令（以 EntryID 透過 Outlook COM 開啟）

`outlook_open_command` 會以「主旨 + 收件時間（預設 ±1 天）」搜尋郵件；
若你想降低誤判，可把 `open_command_match_sender` 設為 `true`，再加上寄件者比對。

> 備註：若郵件來源無法取得 `EntryID`，`outlook_open_command` 欄位會留空。
